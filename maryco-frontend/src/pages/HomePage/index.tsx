...
const features = [
    { icon: <Users className="w-6 h-6 text-blue-600" />, text: 'Групові заняття' },
    { icon: <User className="w-6 h-6 text-blue-600" />, text: 'Індивідуальні заняття' },
    { icon: <BookOpen className="w-6 h-6 text-blue-600" />, text: 'Авторські програми' },
    { icon: <Monitor className="w-6 h-6 text-blue-600" />, text: 'Сучасне обладнання' },
    { icon: <Star className="w-6 h-6 text-blue-600" />, text: 'Індивідуальний підхід' },
]
...
export default function HomePage() {
    ...
    const [submitTrial, { isLoading: isSubmitting, isSuccess, isError }] = useSubmitTrialLessonMutation();

    const handleOpenModal = (id: number | null = null) => {
        setSelectedCourseForModal(id);
        setIsModalOpen(true);
    };
    ...
    return (
        <main className="min-h-screen bg-gray-50 font-nunito">
                ...
                <div className="max-w-6xl mx-auto px-4 py-16 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    <div>
                        <h1 className="text-4xl md:text-5xl font-black text-gray-900 leading-tight mb-8">
                            ...
                            <span className="text-blue-600">своєї дитини</span>
                        </h1>
                        <ul className="flex flex-col gap-4 mb-10">
                            {features.map((f, i) => (
                                <li key={i} className="flex items-center gap-3 font-semibold text-gray-700 text-lg">
                        ...
                        <NavLink
                            to="/courses"
                            className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-black px-10 py-4 rounded-xl text-lg transition-all shadow-lg shadow-blue-200 hover:scale-105"
                        >
                            Обрати курс
                        </NavLink>
                    </div>
            ...
            <section className="py-20 bg-gray-50">
                <div className="max-w-6xl mx-auto px-4">
                    <h2 className="text-4xl font-black text-center text-gray-900 mb-14 uppercase tracking-tight">
                        Чому обирають Maryco Club
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {advantages.map((adv, i) => (
                            <div key={i} className="bg-white rounded-2xl p-8 text-center shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all">
                                <div className="mb-4 flex justify-center">{adv.icon}</div>
                                <h3 className="font-black text-gray-900 text-lg mb-3 uppercase">{adv.title}</h3>
            ...
            <section className="py-20 bg-white">
                <div className="max-w-6xl mx-auto px-4">
                    ...
                    {coursesLoading ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {popularCourses.map(course => (
                                <div key={course.id} className="group bg-gray-50 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col">
                                    <div className="relative h-52 overflow-hidden">
                                        {course.image
                                            ? <img src={course.image} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                            : <div className="w-full h-full bg-gradient-to-br from-blue-100 to-indigo-200 flex items-center justify-center text-6xl">📚</div>
                                        }
                                    </div>
                                    <div className="p-6 flex flex-col flex-1">
                                        <h3 className="font-black text-gray-900 text-lg mb-1 uppercase">{course.title}</h3>
                                        {course.age_range && (
                                            <p className="text-sm text-gray-400 mb-3">Для дітей {course.age_range}</p>
                                        )}
                                        <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                                            <span className="font-black text-blue-600 text-lg">{course.price} грн / міс</span>
                                            <NavLink
                    ...
                    <div className="text-center mt-12">
                        <NavLink
                            to="/courses"
                            className="inline-block border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white font-black px-10 py-4 rounded-xl transition-all"
                        >
                            Всі курси
                        </NavLink>
                    </div>
                    ...
                    {teachersLoading ? (
                        ...
                        <div className="relative flex items-center gap-4">
                            <button
                                ...
                                className="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-600 hover:bg-blue-600 hover:text-white transition-all disabled:opacity-30 flex-shrink-0"
                            >
                                ←
                            </button>
                            <div className="flex gap-6 overflow-hidden flex-1">
                                {teachers.slice(teacherIndex, teacherIndex + visibleTeachers).map(teacher => (
                                    <div key={teacher.id} className="flex-1 min-w-0 bg-white rounded-2xl p-6 text-center shadow-sm hover:shadow-lg transition-all">
                                        <div className="w-20 h-20 rounded-full overflow-hidden mx-auto mb-4 border-4 border-blue-50">
                                            {teacher.photo
                                        ...
                                        <h3 className="font-black text-gray-900 text-sm mb-1">{teacher.full_name}</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {promotions.slice(0, 3).map((promo, index) => (
                                <div
                                    ...
                                    className="group relative flex flex-col bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300"
                                >
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        <h3 className="font-black text-gray-900 text-lg mb-2">{promo.title}</h3>
            ...
            <section className="py-24 bg-gray-900" id="signup">
                    ...
                    <div className="bg-white rounded-[40px] p-10 shadow-2xl">
                        {isSuccess ? (
                            <div className="py-12 text-center">
                                ...
                                <h3 className="text-3xl font-black text-gray-900 mb-2">Заявку прийнято!</h3>
                                <p className="text-gray-500 font-bold">Менеджер незабаром зателефонує вам</p>
                            </div>
                        ) : (
                            <form onSubmit={handleStaticSubmit} className="space-y-5">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <input
                                        ...
                                        placeholder="Ваше ім'я" className="w-full px-6 py-4 bg-gray-50 border-2 border-gray-50 rounded-2xl outline-none focus:border-blue-500 font-bold transition-all"
                                    />
                                    <PatternFormat
                                        format="+380 (##) ###-##-##" allowEmptyFormatting mask="_" required
                                        value={phone} onValueChange={v => setPhone(v.formattedValue)}
                                        className="w-full px-6 py-4 bg-gray-50 border-2 border-gray-50 rounded-2xl outline-none focus:border-blue-500 font-bold transition-all"
                                    />
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <input
                                        type="text" value={age} onChange={e => setAge(e.target.value)}
                                        placeholder="Вік дитини" className="w-full px-6 py-4 bg-gray-50 border-2 border-gray-50 rounded-2xl outline-none focus:border-blue-500 font-bold transition-all"
                                    />
                                    <select
                                        value={courseId || ''} onChange={e => setCourseId(e.target.value ? Number(e.target.value) : null)}
                                        className="w-full px-6 py-4 bg-gray-50 border-2 border-gray-50 rounded-2xl outline-none focus:border-blue-500 font-bold transition-all appearance-none cursor-pointer"
                                    >
                                ...
                                <button
                                    ...
                                    className="w-full py-5 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-2xl tracking-widest transition-all shadow-lg shadow-blue-200"
                                >
