    ...
    return (
        <section className="max-w-6xl mx-auto px-4 py-16 min-h-screen transition-colors">
            <header className="mb-16 text-center">
                <h1 className="text-4xl md:text-5xl font-black text-gray-900 dark:text-white mb-4 transition-colors">Наші викладачі</h1>
                <div className="w-24 h-1.5 bg-blue-600 dark:bg-blue-500 mx-auto rounded-full transition-colors"></div>
                <p className="text-gray-500 dark:text-gray-400 mt-4 text-base md:text-lg transition-colors">Досвідчені педагоги, які надихають на результат</p>
            </header>
                ...
                {teachers.length > 0 ? (
                    teachers.map((teacher) => (
                        <div
                            ...
                            className="group bg-white dark:bg-slate-900 rounded-3xl shadow-md dark:shadow-none hover:shadow-xl border border-gray-100 dark:border-slate-800 transition-all duration-300 overflow-hidden flex flex-col sm:flex-row cursor-pointer hover:-translate-y-1"
                        >
                            {/* Фото викладача */}
                            <div className="relative w-full sm:w-48 h-60 sm:h-auto flex-shrink-0 bg-gray-100 dark:bg-slate-800">
                                {teacher.photo ? (
                                    <img
                                ...
                                {teacher.experience && (
                                    <div className="absolute bottom-3 left-3">
                                        <span className="bg-blue-600/90 backdrop-blur-sm dark:bg-blue-500/90 text-white text-xs font-black px-3 py-1 rounded-full transition-colors shadow-sm">
                                            {teacher.experience}
                                        </span>
                                    </div>
                                )}
                            ...
                            <div className="p-6 flex flex-col justify-between flex-1 min-w-0">
                                <div className="space-y-2">
                                    <h3
                                        className="font-black text-gray-900 dark:text-white text-xl group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-1 break-words"
                                    ...
                                    <div>
                                        <span
                                            className="inline-block max-w-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-black px-3 py-1 rounded-full uppercase tracking-wider transition-colors line-clamp-1 break-words"
                                            title={teacher.subject}
                                        >
                                            {teacher.subject}
                                        </span>
                                    </div>
                                    <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed line-clamp-3 italic transition-colors pt-1 break-words">
                                        {teacher.bio ? `"${teacher.bio}"` : "Інформація про викладача оновлюється."}
                                    </p>
                                </div>

                                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-slate-800 transition-colors">
                                    {teacher.experience ? (
                                        <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 transition-colors">
                                            <span>⭐</span>
                                            <span className="font-semibold line-clamp-1">{teacher.experience}</span>
                                        </div>
                                    ) : <div />}
                                    <span className="text-blue-600 dark:text-blue-400 font-black text-sm group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
                                        Детальніше →
                                    </span>
                                </div>
                            </div>
            ...
            {/* Модальне вікно деталей викладача */}
            {selectedTeacher && (
                <div
